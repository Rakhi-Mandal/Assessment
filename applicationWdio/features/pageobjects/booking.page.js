...
class BookingPage {
 ...
 get selectedHotel(){
  ...
  async handlePopup() {
    await this.signInPopUp.isDisplayed();
  ...
  async filterTheSearchResult() {
    ...
    expect(await this.checkOnlyHotels.isSelected()).to.be.false;
  ...
  async selectAndVerifyHotel(){
    ...
    expect(hotelSelected).to.be.equal( await this.selectedHotel.getText());
    fs.appendFileSync('bookingOutput.txt',`\nHotel verified succesfully`);
