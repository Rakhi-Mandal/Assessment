...
export class YoutubePage {
  ...
  async checkingPageDetails() {
    await helper.assertAllureStep('Validating Website detials(URL,Title,Meta data)', async () => {
    await expect(this.page,"validate page title").toHaveTitle(process.env.youtubePageTitle);
    await expect(this.page,"validate page url").toHaveURL(process.env.youtubeBaseURL);
    expect(this.verifyMetaAttribute,"validate page meta data").toBeVisible;
  });
  }

  async searchingForVideos() {
      await helper.assertAllureStep('Asserting search bar is visible', async () => {
      await expect(this.searchBar,"validate search bar is visible").toBeVisible();
      });
    await helper.assertAllureStep('Asserting search bar is editable', async () => {
    await expect(this.searchBar,"validate search bar is editable").toBeEditable();
    });
    await helper.assertAllureStep('Searching according to requirement and getting the count of recommendations', async () => {
    ...
    expect(count,"validate the found recommendations to be greater than 10").toBeGreaterThanOrEqual(10);
  }); 
}

  async selectVideoFromResults() {
    await helper.assertAllureStep('Asserting video visibility', async () => {
    expect(this.clickAVideo,"validate the selected video is visible").toBeVisible();
    });
    await helper.assertAllureStep('Asserting video enability', async () => {
    expect(this.clickAVideo,"validate the selected video is enabled").toBeEnabled();
    });
    await helper.assertAllureStep('Select a video from obtained results', async () => {
  ...
  async interactWithVideo() {
    await helper.assertAllureStep('validate video play/pause is visible and enabled', async () => {
    expect(this.playPauseButton,"validate video play/pause is visible").toBeVisible();
    expect(this.playPauseButton,"validate video play/pause is enabled").toBeEnabled();
    });
    ...
    await helper.assertAllureStep('validate video progress bar is visible', async () => {
    expect(this.videoProgressBar,"validate video progress bar is visible").toBeVisible();
    });
    await helper.assertAllureStep('validate video setting button is visible', async () => {
    expect(this.videoSettings,"validate video setting button is visible").toBeVisible();
    });
    await helper.assertAllureStep('validate video setting button is enabled then click', async () => {
    expect(this.videoSettings,"validate video setting button is enabled").toBeEnabled();
